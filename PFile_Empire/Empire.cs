...
namespace PFile_Empire
{
    public class Empire
    {
        ...
        public Empire(BBS bbs, BBSDataCore dataInterface)
        {
            _bbs = bbs;
            _bbsDataCore = dataInterface;
        }
        ...
        {
            Taxes();
            EmpireUser.ApplyMinimums(ref CurrentUser);
            Harvest();
            EmpireUser.ApplyMinimums(ref CurrentUser);
            Barbarians();
            EmpireUser.ApplyMinimums(ref CurrentUser);
        ...
        public void News()
        {
            ...
            var s = _bbsDataCore.GetFieldValue(null, "EMPIRENEWS");
            var news = s.Split('|');
            foreach (var ss in news)
            {
        ...
        {
            _bbsDataCore.SetField(null, "EMPIRENEWS", "|" + s);
        }
        ...
        public void Feed()
        {
            ...
            //  //People
            bool promptx = false;
            if (PeasantsRequire > 0)
            {
                if (CurrentUser.Grain <= 0)
                {
                    //No grain to distribute, skip prompts.
                    _bbs.WriteLine("~c1Your people require ~c2" + PeasantsRequire.ToString() + "~c1 bushels ~l1of grain.");
                    _bbs.WriteLine("~c1You don't have jack shit to feed them...");
                    _bbs.WriteLine("~c2SO NOW YOU'RE FUCKED!~c1");
                    _bbs.AnyKey(true, true);
                }
                else
                {
            ...
            if (SoldiersRequire > 0){
                //  //Soldiers
                if (CurrentUser.Grain <= 0)
                {
                    //No grain to distribute, skip prompts.
                    _bbs.WriteLine("~c1Your soldiers require ~c2" + SoldiersRequire.ToString() + "~c1 bushels of grain.");
                    _bbs.WriteLine("~c1You don't have jack shit to feed them...");
                    _bbs.WriteLine("~c2SO NOW YOU'RE FUCKED!~c1");
                    _bbs.AnyKey(true, true);
            ...
            Random r = new Random(DateTime.Now.Millisecond);
            int PeasantsDied = 0;
            if (SoldiersRequire > 0 || PeasantsRequire > 0)
            {
                double PeasantDeficit = 0.0;
                double SoldiersDeficit = 0.0;

                if (SoldiersRequire > 0)
                {
                    SoldiersDeficit = GrainSoldiers / SoldiersRequire;
                }
                if (PeasantsRequire > 0)
                {
                    PeasantDeficit = GrainPeasants / PeasantsRequire;
                }
            ...
            int DiedNaturally = 0;
            int BabiesBorn = 0;
            int PeopleImmigrated = 0;
            int TotalPopulationChange = 0;
            if (CurrentUser.Serfs > 0)
            {
                DiedNaturally = (int)(r.NextDouble() * (CurrentUser.Serfs / NATURAL_DEATH_ADJUST));
                BabiesBorn = (int)(r.NextDouble() * (CurrentUser.Serfs / BABIES_BORN_ADJUST));
                PeopleImmigrated = (int)(r.NextDouble() * (CurrentUser.Serfs / IMMIGRATION_ADJUST));
                TotalPopulationChange = (BabiesBorn + PeopleImmigrated) - (PeasantsDied + DiedNaturally);
            }
            else
            {
                DiedNaturally = 0;
                BabiesBorn = 0;
                PeopleImmigrated = (int)(r.NextDouble() * (100 / IMMIGRATION_ADJUST));
                TotalPopulationChange = PeopleImmigrated;
            }
                ...
                case 3:
                    ...
                    if (CurrentUser.Serfs > 0 || CurrentUser.Land > 0)
                    {
                        if (CurrentUser.Serfs > 0)
                        {
                            ...
                            SerfsBurned = r.Next((int)(CurrentUser.Serfs * 0.2));
                            if (SerfsBurned > 0)
                            {
                        ...
                        {
                            LandBurned = r.Next((int)(CurrentUser.Land / 3));
                            if (LandBurned > 0)
                            {
        ...
        {
            if (
            CurrentUser.Markets > 0 ||
            CurrentUser.Mills > 0 ||
            CurrentUser.Foundries > 0 ||
            CurrentUser.Shipyards > 0 
                )
            {
                Random r = new Random(DateTime.Now.Millisecond);
        ...
        {
            _bbsDataCore.SetField(user.UserId, "EMPIRE",EmpireUser.Serialize(user));
        }

        private void SaveUser()
        {
            _bbsDataCore.SetField(CurrentUser.UserId, "EMPIRE", EmpireUser.Serialize( CurrentUser));
        }

        private void DeleteUser(int UserId)
        {
            //Stupid fuck died.
            _bbsDataCore.SetField(UserId, "EMPIRE", "");
        }

        private List<EmpireUser> ListUsers()
        {
            List<EmpireUser> ulist = new List<EmpireUser>();
            List<Field> fields = _bbsDataCore.GetAllFieldsWithFieldName("EMPIRE");
            foreach (var field in fields)
            {
                ulist.Add(EmpireUser.Deserialize(field.FieldContents));
            }
            return ulist;
        }
        ...
        private void LoadUser()
        {
            ...
            {
                CurrentUser = EmpireUser.Deserialize(dustr);
